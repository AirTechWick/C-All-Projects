...
void MovieTicketMaster::Init() 
{
    ...
    // Next set the pointer to the first element in the array of Movie which is one of the members of the class. If you tempt to declare a Movie array locally in this Init function it's an error. Don't do it. Note that this Init function is a member function of the MovieTicketMaster class. Therefore it should have direct access to the member data Movie array.

    p_movie_object = &movie_objects_[0];

    // Next define three pointers that will be used to access and iterate thru the three arrays movie_names, seats and prices. Of course you must also set them to the first element in each array accordingly.
...
Movie * MovieTicketMaster::FindMovie(string movie_name)
{
    ...
    string * p_movie_name = nullptr;

    Movie * p_movie_object = nullptr;

    p_movie_name = &movie_name;
...
void MovieTicketMaster::PurchaseTicket()
{
    ...
    Movie * found_movie = FindMovie(s_user_response);
